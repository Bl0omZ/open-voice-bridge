...
**下载：** [小米遥控器桥接 0.3.1（macOS，universal）](https://github.com/Bl0omZ/open-voice-bridge/releases/latest)
...
<p align="center">
  <img src="docs/images/settings-buttons.png" width="1000" alt="小米蓝牙遥控器 2 Pro / RC003 图形化按键映射设置界面">
</p>

<p align="center"><sub>“按键”设置界面：点击左侧实物按键，右侧会定位到对应动作；单击和长按修改后自动保存。</sub></p>

打开菜单栏中的应用图标，选择“打开设置… → 按键”，即可使用图形化映射：

1. 打开“启用 RC003 自定义按键映射”；修改或录制按键时也会自动启用；
2. 选择“通用”、“Codex”或“Claude Code”配置；
3. 点击左侧实物图上的按键，右侧会自动定位到对应映射；
4. 分别设置单击和长按动作，修改后自动保存；
5. 点击动作末尾的键盘图标，可以录制一个普通键或带 `Control`、`Option`、`Shift`、`Command` 的组合键；按 `Esc` 取消；
6. 需要撤销当前配置时，点击“恢复默认”；只恢复当前选中的配置，不影响另外两套。

麦克风键固定用于 **RC003 语音 + Mac Fn/🌐︎ 长按**，不参与普通按键下拉映射。遥控器没有独立静音实体键；“系统静音”只是可分配给其他实体键的可选动作，不是默认映射。

macOS 会优先处理 `Command-Tab`、`Command-Space` 等系统快捷键，设置页可能无法录制这些组合。`Command-Tab` 可以直接选择现有的“应用切换”动作。

没有长按动作时，方向、返回和音量的预置动作继续支持连发。配置了长按动作后，1 秒内松开执行单击；按满 1 秒只执行长按。

#### 自动选择配置

- Codex App 使用 Codex 配置；
- 设置页勾选的终端应用使用 Claude Code 配置，默认勾选 Ghostty 和 Warp；
- 其他应用使用通用配置。

App 只能识别前台宿主应用。勾选某个终端后，该终端中的普通 shell 标签页也会使用 Claude Code 配置。

新版会继续读取 v0.2.0 的按键配置。保存自定义快捷键后，旧版 App 可能无法读取新版配置；降级前请使用“恢复默认”。

#### 默认映射

| 按键 | 通用单击 | 通用长按 | Codex 单击 | Codex 长按 | Claude Code 单击 | Claude Code 长按 |
| --- | --- | --- | --- | --- | --- | --- |
| 麦克风 | 固定语音 + Fn/🌐︎ | 固定语音 + Fn/🌐︎ | 固定语音 + Fn/🌐︎ | 固定语音 + Fn/🌐︎ | 固定语音 + Fn/🌐︎ | 固定语音 + Fn/🌐︎ |
| 电源 | Escape | 无 | Escape | 无 | Escape | 无 |
| 上 / 下 / 左 / 右 | 对应方向键 | 连发 | 对应方向键 | 连发 | 对应方向键 | 连发 |
| OK | Enter | 无 | Enter | 无 | Enter | `Control-J` 换行 |
| 返回 | Delete | `Command-Delete` 清行 | Delete | `Command-Delete` 清行 | Delete | `Control-U` 清行 |
| 音量 + | 系统音量 + | 无 | 上一个会话 | 后退 | `Control-R` 历史搜索 | `Control-B` 后台运行 |
| 音量 - | 系统音量 - | 无 | 下一个会话 | 前进 | `Control-T` 任务清单 | `Control-S` 暂存输入 |
| 主页 | 显示桌面 | 无 | 新建会话 | 命令菜单 | `Control-C` 中断 | `Option-O` 快速模式 |
| 菜单 | 右键菜单 | 无 | 打开审查 | 切换审查面板 | `Shift-Tab` 权限模式 | `Option-T` 思考模式 |
| TV | 应用切换 | 无 | 搜索会话 | `Control-Tab` 最近会话 | `Control-O` 详细记录 | `Option-P` 模型选择 |
...
1. 从 [0.3.1 Release](https://github.com/Bl0omZ/open-voice-bridge/releases/tag/v0.3.1) 下载 `xiaomi-remote-bridge-mac-0.3.1.dmg`；
...
正式版和旁装测试版只能运行一个。两个 App 同时运行会争用同一台 RC003，导致按键或语音行为不稳定。旁装测试 App 使用临时签名，重新构建或替换后，macOS 可能要求重新授予“输入监控”和“辅助功能”权限。
...
<p align="center">
  <img src="docs/images/settings-permissions.png" width="1000" alt="权限设置界面：蓝牙、输入监控、辅助功能均已授权">
</p>
...
权限开关已打开、应用仍提示未授权时，通常是 App 被重新构建或替换后，旧权限记录仍指向上一个二进制文件：完全退出应用，在“输入监控”和“辅助功能”中删除旧条目，用 `+` 重新加入当前 App 并打开两个权限，再重新启动。

#### 5. 配置 BlackHole 语音链路

<p align="center">
  <img src="docs/images/settings-connection.png" width="1000" alt="连接设置界面：已连接小米蓝牙语音遥控器，语音输出选择 BlackHole 2ch">
</p>

1. 安装 [BlackHole 2ch](https://existential.audio/blackhole/)（已安装 Homebrew 时可执行 `brew install blackhole-2ch`）；
...
macOS 的系统声音输出继续使用扬声器或耳机，不要改成 `BlackHole 2ch`，否则可能听不到系统声音。`BlackHole 2ch` 没有出现在设备列表中时，先在“音频 MIDI 设置”中确认设备存在，关闭正在使用音频的应用后重新打开本应用；仍未出现时重启 Mac 再检查。
...
- **TV 键只输入反引号**：RC003 的 TV 键会被 macOS 原生识别为反引号键，取得输入监控权限后应用会拦截原始字符再发送配置的动作。出现反引号时，依次确认自定义按键映射已启用、两项按键权限授予了当前 App、正式版和测试版没有同时运行、App 在授权后没有被重新构建替换。
- **按键执行的快捷键与设置不符**：先确认当前前台应用对应哪套配置——Codex 使用“Codex”配置，勾选的终端和编辑器使用“Claude Code”配置，其他应用使用“通用”配置。修改“通用”不会影响另外两套。

### 查看日志

打开“设置 → 权限”，点击“在 Finder 中显示日志”。日志文件位于 `~/Library/Logs/XiaomiRemoteBridgeMac/runtime.log`。排查按键时重点查看：
