...
4. 点击每行末尾的键盘图标，可以录制一个普通键或带 `Control`、`Option`、`Shift`、`Command` 的组合键；按 `Esc` 取消；
5. 需要撤销所有自定义设置时，点击“恢复默认”。

麦克风键固定用于 **RC003 语音 + Mac Fn/🌐︎ 长按**，不参与普通按键下拉映射。遥控器没有独立静音实体键；“系统静音”只是可分配给其他实体键的可选动作，不是默认映射。

macOS 会优先处理 `Command-Tab`、`Command-Space` 等系统快捷键，设置页可能无法录制这些组合。`Command-Tab` 可以直接选择现有的“应用切换”动作。自定义快捷键每次按下只触发一次；方向、返回和音量的预置动作继续支持长按连发。

#### Codex App / Claude Code 推荐映射

| 遥控器按键 | 建议动作 | 用途 |
| --- | --- | --- |
| 麦克风 | 固定语音 + Fn/🌐︎ | 向当前输入框口述内容 |
| TV | 自定义 `Command-G` | Codex App 搜索会话，使用方向键选择并按 OK 确认 |
| 菜单 | 自定义 `Control-Tab` | Codex App 直接切换到下一个最近会话；松开 Control 后完成切换 |
| 上 / 下 / 左 / 右 | 现有方向键预置 | 在会话列表和界面中移动 |
| 确定（OK） | 现有 Return 预置 | 选择会话或发送输入 |
| 返回 | 现有 Delete 预置 | 删除输入内容 |
| 电源 | 现有 Escape 预置 | 取消当前操作 |
| 主页 | 自定义 `Control-O` | Claude Code 打开详细记录 |
